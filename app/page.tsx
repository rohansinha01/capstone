...
export default function Home() {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      setMedia((prevMedia) => prevMedia?.filter((i) => i.$id !== id));
    } catch (error) {
            ...
            {media?.map((aMedia) => (
              <div key={aMedia.$id}>
                ...
                >
                  ...
                  <div>
                    Score: {aMedia.characters + aMedia.story + aMedia.vibes}
                  </div>
                  <div></div>
                </div>
                <div className="flex gap-4 mt-4 justify-start">
                  <Link
                    className="bg-green-400 px-4 py-2 rounded-md uppercase text-sm font-bold tracking-widest"
                    href={`/show/${aMedia.$id}`}
                  >
                    show
                  </Link>
